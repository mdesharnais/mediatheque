...
Nous pouvons toujours être sûrs que, pour chaque enregistrement de la table \texttt{media}, il y aura toujours, selon le type, un enregistrement correspondant dans la table \texttt{audios\_videos} ou bien dans la table \texttt{imprimes}.
...
Pour ajouter à l'ergonomie déjà présente dans le fil d'Ariane, nous avons établi que s'il y avait plus de trois suggestions de valeurs pour un critère, une entrée de liste "Afficher plus" serait affichée pour développer les valeurs supplémentaires. La page est donc moins surchargée. Il est à noter que cette fonction n'est disponible que sur un navigateur ayant le JavaScript activé. La désactivation du JavaScript n'amène pas de problèmes en tant que tel. Les valeurs des critères seront tout simplement affichées dans leur ensemble sans possibilité de cacher X résultats.
...
\begin{itemize}
	...
	\item Poids de la base de données réduit puisqu'il est moins lourd de définir de nombreuses clés étrangères que de répéter de nombreuses fois la même chaine de caractères.
...
\begin{enumerate}
	\item Bandeau
	\item Fil d'Ariane vertical (n'apparait cependant que dans certaines pages)
...
Le fil d'Ariane vertical permet d'afficher des critères permettant davantage la recherche. Il contient des liste de critères permettant de raffiner la recherche.
...
\begin{table}[h!tbp]
	...
	\begin{center}
		...
		\includegraphics[scale=0.2]{black_arrow_big.png} & 370x216, 209x122 & Cette image servira à afficher les détails d'un média sous forme d'infobulle.\\ \hline
...
\begin{figure}[h]
	\begin{center}
		\includegraphics[scale=0.3]{captures_ecran/modele_details_medias.png}
	\end{center}
	\caption{Modèle détails d'un média}
\end{figure}

\begin{figure}[h]
	\begin{center}
		\includegraphics[scale=0.3]{captures_ecran/modele_details_avances_medias.png}
	\end{center}
	\caption{Détails avancés d'un média audio}
\end{figure}
...
\begin{figure}[htbp]
	\begin{center}
		\includegraphics[scale=0.3]{captures_ecran/modele_rapport.png}
	\end{center}
	\caption{Modèle de rapports}
\end{figure}
...
Le diagramme de classe est disponible dans la documentation Doxygen se trouvant sur le CD accompagnant ce document. Pour pouvoir la consulter, il suffit d'ouvrir le fichier index.html et elle s'ouvrira sous forme de site internet.
...
Le prototype fonctionnel est disponible sur le CD accompagnant ce document.
