...
Des fonctions de recherche simple et avancées est un élément essentiel pour un système de médiathèque. Le nôtre n'en fait pas exception puisqu'il un module de recherche évolué et des possiblités de raffinement sans limite. Nous avons voulu créer un système nécessitant un système de recherche nécessitant un temps d'apprentissage quasi-nul. Pour cela, nous avons joint quelques particularités aux fonctions de recherche de base. Voici la liste des éléments assurant une facilité de recherche et une bonne naviguabilité des résultats correspondants à cette recherche.
...
La police de caractère que nous avons choisis pour le site est Optima. Conformément à la convention, nous avons définis une liste de polices alternatives qui seront utilisées si Optima n'est pas disponnible. La liste est, par ordre de priorité~: Trebuchet MS, Lucida, Arial, Geneva, Verdana, Lucida Grande, Tahoma, Helvetica, sans-serif.

Nous avons décidés de ne pas donner une taille fixe à la police de caractère mais plutôt d'utiliser la taille désirée par l'utilisateur, définie dans les propriétées de son navigateur. Par exemple, pour configurer la taille de police par défaut sous Mozilla Firefox, il suffit d'aller dans \texttt{Préférences > Contenu > Polices et couleurs > Taille} et de définir la taille souhaitée. Nous sommes partis du principe que, si l'utilisateur à choisi d'afficher ses sites Internet avec une certaine taille, ce n'est pas à nous de contrevenir à sa volonté.
...
\begin{figure}[htbp]
	\begin{center}
		\includegraphics[scale=0.5]{captures_ecran/modele_descriptif.png}
	\end{center}
...
\begin{figure}[htbp]
	\begin{center}
		\includegraphics[scale=0.5]{captures_ecran/modele_resultats_recherche.png}
	\end{center}
...
\begin{figure}[htbp]
	\begin{center}
		\includegraphics[scale=0.5]{captures_ecran/modele_tables_de_pilotage.png}
	\end{center}
...
\begin{figure}[htbp]
	\begin{center}
		\includegraphics[scale=0.5]{captures_ecran/modele_formulaire_simple.png}
	\end{center}
...
\begin{figure}[htbp]
	\begin{center}
		\includegraphics[scale=0.5]{captures_ecran/modele_details_medias.png}
	\end{center}
	\caption{Modèle formulaire simple}
\end{figure}
