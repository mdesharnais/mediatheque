...
if ($r != Null) {
	$sql="SELECT medias.ID as ID, medias.reference as reference, medias.titre as titre, medias.annee_publication as annee_publication, medias.notes as notes, medias.image as image, maisons_edition.nom as menom, categories.nom as cnom, emprunts.date_reservation as date_reservation FROM medias 
	...
	foreach($query as $row)
	{
		...
		echo "<a href='media.php?ID=" . $row['ID'] . "'><p id='" . $row['reference'] . "aazz'>" . $row['reference'] . " | " . $row['titre'] . " reserve pour le " . $row['date_reservation'] . "</p></a>";
